...
export default function ProductsShowcase() {
  const ref = useScrollAnimation<HTMLDivElement>({ childSelector: '.product-card', stagger: 0.1 });

  return (
    <section id="products" className="section-padding bg-secondary/50">
      <div className="section-container">
        <div className="text-center max-w-2xl mx-auto mb-16">
          <span className="text-accent font-semibold text-sm uppercase tracking-widest mb-3 block">Our Products</span>
          <h2 className="text-3xl md:text-4xl font-heading font-bold text-foreground mb-4">
            Genuine Equipment From Global Brands
          </h2>
          <p className="text-muted-foreground text-lg">
            Authorized dealer for the industry's most trusted names. Every product backed by manufacturer warranty.
          </p>
        </div>
          ...
          {products.map((product) => (
            ...
            >
              ...
              <div className="p-6">
                ...
                <p className="text-muted-foreground text-sm">{product.benefit}</p>
              </div>
            </div>
          ))}
        </div>
